...
As we've seen in the above, we can query the `node` field on `Query` with the globally unique `key`, a concrete
example of that would look like the following:
...
Where a correct `variables.id` would look like `VXNlcjox` which decodes to `User:1`. No worries though, we'll have
all of this available in the output of lists/... to use in your subsequent queries.
